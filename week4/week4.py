...
df1 = pd.DataFrame(data=dataset, columns=[description[0], 'Years'])
df2 = pd.DataFrame(df1['Years'].to_list(), columns=description[1])
df3 = pd.concat([df1['Country'], df2], axis=1)


def get_year_data(frame, year):
    l = []
    for i, j in zip(frame['Country'], frame[year]):
        l.append((i, j))
    return {year: l}


# print(df3['2019 '].tolist())
# print(get_year_data(df3, '2019 '))


def get_country_data(frame, country):
    row = frame.loc[frame['Country'] == country]
    l = []
    for i, j in zip(row.columns, row.iloc[0]):
        l.append((i, j))
    return {'Romania': l[1:]}


print(get_country_data(df3, 'RO'))
